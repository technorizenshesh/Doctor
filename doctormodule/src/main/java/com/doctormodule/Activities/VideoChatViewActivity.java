...
public class VideoChatViewActivity extends AppCompatActivity {
    ...
    private final IRtcEngineEventHandler mRtcEventHandler = new IRtcEngineEventHandler() {
        ...
        public void onJoinChannelSuccess(String channel, final int uid, int elapsed) {
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    Log.e("Join success, uid: " , ""+(uid & 0xFFFFFFFFL));
                }
            });
        }
        ...
        public void onFirstRemoteVideoDecoded(final int uid, int width, int height, int elapsed) {
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    Log.e("First remote, uid: " , ""+(uid & 0xFFFFFFFFL));
        ...
        public void onUserOffline(final int uid, int reason) {
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    Log.e("User offline, uid: ",""+(uid & 0xFFFFFFFFL));
                    onRemoteUserLeft();
                }
            });
        }
    ...
    private void joinChannel() {
        ...
        String chenal_id="";
        if (data!=null){
            chenal_id="Ch"+data.getId();
        }else {
            chenal_id="Ch"+req_data.getId();
        }
        mRtcEngine.joinChannel(token, chenal_id, "Extra Optional Data", 0);
    }

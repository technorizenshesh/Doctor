...
//        Uri uri = Uri.parse("market://details?id=" + BuildConfig.APPLICATION_ID);
  ...
  */  public void isLocationEnabled(final Context mContext, LocationManager locationManager) {
    ...
    public File getFile(Context context,Bitmap bitmap){
...
//write the bytes in file
    ...
    public int getSelectedPos(String[]list,String txt){
        for (int i=0;i<list.length;i++){
            if (list[i].equals(txt)){
                return i;
            }
        }
        return 0;
    }public int getSelectedPos(ArrayList<String>list,String txt){
        for (int i=0;i<list.size();i++){
            if (list.get(i).equals(txt)){
                return i;
            }
        }
        return 0;
